...
Adafruit_ADS1115 ads(0x48); //Con Adafruit_ADS1115 especificamos que es la versión de 16 bit.
LiquidCrystal_I2C lcd(0x27, 16, 2); //Especificamos que la dirección del LCD es 0x27
...
double calcIrms(unsigned int Number_of_Samples){
  ...
  for (unsigned int n = 0; n < Number_of_Samples; n++)
  {
    ...
    // Elimina los 1,6V que tiene el sistema haciendo que la muestra esté centrada en 0 
...
void mandarJson(double irms, double potencia){
   ...
   if (WiFi.status() == WL_CONNECTED) { //Comprobar wifi
    ...
    values.add(irms); //Añadir valor al array
    values.add(potencia); //Añadir valor al array

    JsonArray& valuesInfo = JSONencoder.createNestedArray("valuesInfo"); //JSON array
    valuesInfo.add("irmsnuevo"); //Añadir valor al array
    valuesInfo.add("potencianueva"); //Añadir valor al array
    ...
    Serial.println(httpCode);   //Imprime el código http
